...
exports.generateDynamicKeyName = exports.encodeDynamicKeyPart = exports.dynamicTypesRegex = void 0;
...
exports.dynamicTypesRegex = /<(uint|int|bytes)(\d+)>/;
/**
 *
 ...
 */
const encodeDynamicKeyPart = (type, value_, bytes) => {
    let baseType = '';
    let size = 0;
    let value = value_;
    if (dynamicTypes.includes(type)) {
        ...
        const regexMatch = type.match(exports.dynamicTypesRegex);
        if (!regexMatch) {
    ...
    switch (baseType) {
        ...
        case 'address': {
            ...
            return (0, web3_utils_1.padLeft)(value.slice(0, 2 + bytes * 2), bytes * 2)
                .slice(2)
        ...
        case 'uint': {
            ...
            let hex = (0, web3_utils_1.numberToHex)(value).slice(2);
            ...
            return (0, web3_utils_1.padLeft)(hex, bytes * 2).slice(2);
        }
        case 'int':
            // TODO:
        ...
        case 'bytes': {
            ...
            return (0, web3_utils_1.padRight)(value.slice(0, 2 + bytes * 2), bytes * 2).slice(2);
        }
        default:
            throw new Error(`Dynamic key: ${type} is not supported`);
    }
...
function isDynamicKeyName(name) {
    ...
    for (let i = 0; i < keyNameParts.length; i++) {
        if (dynamicTypes.includes(keyNameParts[i]) ||
            keyNameParts[i].match(exports.dynamicTypesRegex)) {
            return true;
        }
    }
    return false;
...
const encodeDynamicMappingWithGrouping = (name, dynamicKeyParts) => {
    ...
    if (keyNameSplit[1].startsWith('0x')) {
        secondPart = (0, web3_utils_1.padRight)(keyNameSplit[1].slice(0, 2 + 4 * 2), 4 * 2).slice(2);
    }
    ...
    if (keyNameSplit[2].startsWith('0x')) {
        lastPart = (0, web3_utils_1.padRight)(keyNameSplit[2].slice(0, 2 + 20 * 2), 20 * 2).slice(2);
    }
...
function encodeKeyName(name, dynamicKeyParts) {
    ...
    switch (keyType) {
        ...
        case 'Mapping': {
            const keyNameSplit = name.split(':');
            if ((0, web3_validator_1.isAddress)(keyNameSplit[1])) {
        ...
        case 'Array': // Warning: this can not correctly encode subsequent keys of array, only the initial Array key will work
            ...
            if (dynamicKeyParts && typeof dynamicKeyParts === 'number') {
                return (0, utils_1.encodeArrayKey)((0, web3_utils_1.keccak256)(name), dynamicKeyParts);
            }
...
const generateDynamicKeyName = (name, dynamicKeyParts) => {
    ...
    return name
        ...
        if (keyNamePart === '<address>') {
            if (!(0, web3_validator_1.isAddress)(dynamicKeyPart)) {
